...
export default function CreateBusinessPage() {
  ...
  // Client-side field values (needed for the Ready state display + validation)
  const [businessName, setBusinessName] = useState("");
  const [currency, setCurrency] = useState("NGN");
  const [businessType, setBusinessType] = useState("Retail");

  // Blur-touch tracking for inline validation UX
  ...
  // Currencies loaded from public JSON
  ...
  // useActionState — wires form directly to server action
  const [state, formAction, isPending] = useActionState<ActionState, FormData>(
    createBusinessAction,
    null,
  );

  // Ready = action returned success
  ...
  // Prevent submit if client errors exist (server action still validates too)
    ...
    <main className="min-h-screen w-full bg-[#ffffff] flex flex-col items-center justify-center px-6 font-[family-name:var(--font-geist-sans)] overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        >
          ...
          >
            {/* Hidden fields so formData contains correct names matching the server action */}
